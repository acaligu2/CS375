...
int main(int argc, char const *argv[]) {
  ...
  if(argc != 4){
    cout << "./program4 <inputFile> <outputFile> <0 or 1 or 2 or 3>" << endl;
    exit(1);
  }
  ...
  if(functionOption != 0 && functionOption != 1 && functionOption != 2 && functionOption != 3){
    cout << "Invalid function request: select 0/1/2/3" << endl;
    exit(1);
  }
  ...
  for(int i = 0; i < problems.size(); i++){
    ...
    if(opt == 0){
      ...
      seconds = (funcTime / CLOCKS_PER_SEC);
    ...
    //INITIALIZE CODE FOR DYNAMIC PROGRAMMING METHOD
    }else if(opt == 3){

      start = clock();

      problems[i]->dynamicProgramming();
      end = clock();
      float funcTime = ((float)end - (float)start);
      seconds = (funcTime / CLOCKS_PER_SEC);
    ...
    //Format output
    problems[i]->writeToOutput(seconds, opt);
